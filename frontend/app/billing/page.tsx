...
async function openRazorpayCheckout(res: {
  key_id: string;
  ...
  await new Promise<void>((resolve, reject) => {
    const rzp = new RazorpayCheckout({
      ...
      prefill: { email: res.prefill_email },
      handler: async (response: any) => {
        try {
          await api("/billing/verify", { method: "POST", body: JSON.stringify(response) });
...
export default function BillingPage() {
  ...
  const checkout = async (plan: string) => {
    ...
    setError(null);
    try {
      const res = await api<{ key_id: string; subscription_id: string; name: string; description: string; prefill_email: string }>(
        "/billing/checkout",
      ...
      <DashboardShell>
        ...
        {loading ? (
          ...
          <>
            {sub.trial_active ? (
              <div className="mb-6 rounded-lg border border-brand-200 bg-brand-50 p-4 text-sm text-brand-800">
                <p className="font-semibold">
                  Free trial: {sub.trial_days_remaining} day{sub.trial_days_remaining === 1 ? "" : "s"} remaining — 200 quotes &amp; 200 AI assists included
                </p>
            ...
            <Card title="Manage" action={sub.plan !== "free" ? <button className="text-sm text-brand-600 hover:underline" onClick={() => window.open("https://dashboard.razorpay.com/", "_blank")}>Razorpay dashboard</button> : undefined}>
              {sub.plan === "free" ? (
