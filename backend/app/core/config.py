...
class Settings(BaseSettings):
    ...
    email_from: str = "hello@quoteflowai.in"
    support_email: str = "hello@quoteflowai.in"
    ...
    razorpay_success_url: str = "http://localhost:3000/billing?checkout=success"
    razorpay_cancel_url: str = "http://localhost:3000/billing?checkout=cancelled"
    ...
    def validate_production(self) -> None:
        ...
        required = {
            ...
            "RAZORPAY_STARTER_PLAN_ID": self.razorpay_starter_plan_id,
        ...
        razorpay_plan_ids = [
            price_id
            for price_id in (
                self.razorpay_starter_plan_id,
                self.razorpay_pro_plan_id,
                self.razorpay_business_plan_id,
            )
            if price_id
        ]
        if len(set(razorpay_plan_ids)) != len(razorpay_plan_ids):
            print(
                "[FATAL] Production startup aborted: Razorpay Basic/Pro/Business Plan IDs must be unique.",
                file=sys.stderr,
            )
            raise RuntimeError("Razorpay Plan IDs must be unique across all paid plans.")
