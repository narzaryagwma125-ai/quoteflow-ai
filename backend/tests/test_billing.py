...
@pytest.mark.asyncio
async def test_active_subscription_wins_over_newer_incomplete_subscription():
    user = await create_user("priority@example.com")
    async with async_session_factory() as db:
        db.add(
            Subscription(
                user_id=user.id,
                provider="stripe",
                provider_customer_id="cus_business",
                provider_subscription_id="sub_business",
                plan="business",
                status="active",
            )
        )
        ...
        db.add(
            Subscription(
                user_id=user.id,
                provider="stripe",
                provider_customer_id="cus_starter",
                provider_subscription_id="sub_starter",
                plan="starter",
                status="incomplete",
            )
        )
...
@pytest.mark.asyncio
async def test_sync_stripe_subscriptions_repairs_existing_paid_subscription(client, monkeypatch):
    from app.core.config import settings as cfg

    monkeypatch.setattr(cfg, "stripe_business_price_id", "price_business")
    user = await create_user("sync@example.com")
    async with async_session_factory() as db:
        db.add(
            Subscription(
                user_id=user.id,
                provider="stripe",
                provider_customer_id="cus_sync",
                provider_subscription_id="sub_sync",
                plan="business",
                status="incomplete",
            )
        )
    ...
    monkeypatch.setattr("app.api.billing.stripe.Subscription", FakeStripeSubscription)
    await login(client, "sync@example.com")
    res = await client.post("/api/billing/sync-stripe-subscriptions")
    assert res.status_code == 200, res.text
    assert res.json()["status"] == "synced"
    assert res.json()["synced"][0]["status"] == "active"

    async with async_session_factory() as db:
        sub = (
            await db.execute(
                select(Subscription).where(Subscription.provider_subscription_id == "sub_sync")
            )
        ).scalar_one()
        assert sub.status == "active"
        assert sub.plan == "business"
